...
const Trashcan = () => {
  ...
  const TaskColumn = ({ tasks = [] }) => (
    <div className={styles.trashcanColumn}>
      {tasks.map((task, index) => (
        <div key={index} className={styles.trashcanItem}>
          <span>{task.text}</span>
          <button
  ...
  if (!trashedTasks || trashedTasks.length === 0) {
    return <div className={styles.emptyTrashcan}>Trashcan is empty</div>;
  }

  return (
    <div className={styles.trashcan}>
      <div className={styles.trashcanControls}>
        <h3 className={styles.trashcanHeader}>Trashcan</h3>
        <button
          className={styles.deleteAllButton}
          onClick={handleClearTrashcan}
          title='Empty trashcan'
        >
          <svg className={styles.trashcanIcon} viewBox='0 0 24 24'>
